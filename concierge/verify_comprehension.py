...
from . import comprehension, corpus, db, engine, store
...
def run(r) -> None:
    ...
    # Every class the engine can detect must actually be exercised, and every phrasing written
    # here must actually trigger the class it was written for. Without this the two files drift:
    # a qualifier class gets added to the engine, nobody writes questions for it, and the suite
    # keeps passing while a whole category of client question goes untested.
    untested = [c for c, phrasings in corpus.QUALIFIER_CLASSES.items() if not phrasings]
    undetected = []
    for cls, phrasings in corpus.QUALIFIER_CLASSES.items():
        for phrasing in phrasings:
            seen = comprehension.assess({}, f"How much is Widget {phrasing}?",
                                        service_name="Widget", matched_on=("widget",))
            if cls not in seen.qualifiers:
                undetected.append((cls, phrasing))

    r.check(
        "The corpus contains no trade vocabulary, and exercises every class the engine detects",
        not offenders and not untested and not undetected,
        ...
        f"| qualifier classes the engine detects: {', '.join(sorted(comprehension.QUALIFIERS))}\n"
        f"| classes with no questions written for them: {untested or 'none'}\n"
        f"| phrasings the engine failed to detect as their own class: {undetected or 'none'}",
    )
